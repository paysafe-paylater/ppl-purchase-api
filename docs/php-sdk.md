# Introduction
The Paysafe Pay Later PHP SDK provides a simplified way to integrate with the Paysafe Pay Later instore purchase API. The source of this SDK can be found on [Github](https://github.com). This documentation contains information on how to get started with the Paysafe Pay Later PHP SDK.
...
$configuration = new ConnectionConfiguration(ApiConnection::TEST_URL);
...
---------|----------|---------
  baseUrl | | The API base url. Use the constants `ApiConnection::URI_TEST` or `ApiConnection::URI_LIVE`.
...
$client = new Client([
    'base_uri' => ApiConnection::URI_TEST,
    'timeout'  => 15.0,
]);
...
$connection = ApiConnection::create(
    new ConnectionConfiguration(ApiConnection::URI_TEST)
);
$communicator = new PaySafePayLaterCommunicator($connection);
```
